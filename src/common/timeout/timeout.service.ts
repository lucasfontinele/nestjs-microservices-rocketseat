...
export class TimeoutService {
  ...
  async executeWithRetry<T>(
    ...
    this.logger.error(`All ${options.retries + 1} attempts failed. Last error: ${lastError!.message}`);
    throw lastError!;
  }
  ...
  async executeWithCustomTimeout<T>(
    ...
    return Promise.race([operation(), this.createTimeoutPromise(timeoutMs)]) as Promise<T>;
  }
}
